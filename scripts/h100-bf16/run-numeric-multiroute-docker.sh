...
#   bash scripts/h100-bf16/run-numeric-multiroute-docker.sh
set -Eeuo pipefail
...
image_tag="akbc-qwen27b-bf16-val:numeric-multiroute"
...
docker run "${run_args[@]}" "${image_tag}" \
  /opt/akbc/scripts/h100-bf16/run-numeric-multiroute-container.sh

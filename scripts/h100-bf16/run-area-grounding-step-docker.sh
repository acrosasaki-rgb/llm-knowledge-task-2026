...
#   bash scripts/h100-bf16/run-area-grounding-step-docker.sh
set -Eeuo pipefail
...
image_tag="akbc-qwen27b-bf16-val:area-grounding-step"
...
docker run "${run_args[@]}" "${image_tag}" \
  /opt/akbc/scripts/h100-bf16/run-area-grounding-step-container.sh

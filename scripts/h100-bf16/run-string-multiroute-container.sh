#!/usr/bin/env bash
# Multi-route elicitation for the four string relations on a multi-GPU host:
# all 278 validation rows (awardWonBy, countryLandBordersCountry,
# companyTradesAtStockExchange, personHasCityOfDeath) x 20 candidates, four
# relation-specific reasoning routes x five samples each, raw thinking
# persisted. One llama-server per GPU (the BF16 27B fits on a
# single 80 GB device), AKBC_CLIENTS_PER_GPU shard clients per server.
set -Eeuo pipefail

cd /opt/akbc

model_key="${AKBC_MODEL_KEY:-qwen3.5-27b-bf16-string-multiroute}"
config="${AKBC_CONFIG:-configs/experiment-qwen-3.5-27b-bf16-string-multiroute.yaml}"
...
with open(val_path, encoding="utf-8") as stream:
    for line in stream:
        ...
        if row["Relation"] in (
            ...
            selected.append(row)
if len(selected) != 278:
    raise SystemExit(f"expected 278 string-relation rows, found {len(selected)}")
with open(output_path, "w", encoding="utf-8") as stream:
    for row in selected:
        stream.write(json.dumps(row, ensure_ascii=False) + "\n")
print(f"selected {len(selected)} string-relation rows")
PY
...
echo "string-multiroute wall time: ${elapsed} s"
echo "string-multiroute completed: outputs/${model_key}-candidates.jsonl"

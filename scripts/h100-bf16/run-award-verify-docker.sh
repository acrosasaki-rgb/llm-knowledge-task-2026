...
#   bash scripts/h100-bf16/run-award-verify-docker.sh
set -Eeuo pipefail
...
image_tag="akbc-qwen27b-bf16-val:award-verify"
...
for variable_name in \
  AKBC_MODEL_KEY AKBC_CONFIG AKBC_PARALLEL AKBC_CLIENTS_PER_GPU \
  AKBC_CTX_PER_SLOT AKBC_POOL_PATH AKBC_VERIFY_VOTES HF_TOKEN \
...
docker run "${run_args[@]}" "${image_tag}" \
  /opt/akbc/scripts/h100-bf16/run-award-verify-container.sh

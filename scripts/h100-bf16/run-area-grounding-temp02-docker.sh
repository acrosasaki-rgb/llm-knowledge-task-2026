...
#   bash scripts/h100-bf16/run-area-grounding-temp02-docker.sh
set -Eeuo pipefail
...
image_tag="akbc-qwen27b-bf16-val:area-grounding-temp02"
...
docker run "${run_args[@]}" "${image_tag}" \
  /opt/akbc/scripts/h100-bf16/run-area-grounding-temp02-container.sh

#!/usr/bin/env bash
# Think-content sample: regenerate 12 hasArea validation rows x 20 candidates
# with the full raw output (thinking included) persisted into the candidate
# diagnostics. Sampling is identical to the BF16 20-candidate baseline;
# candidate seeds derive from (seed, subject, relation, candidate_index), so
# the selected rows reproduce the baseline pool.
set -Eeuo pipefail

cd /opt/akbc

model_key="${AKBC_MODEL_KEY:-qwen3.5-27b-bf16-think-sample}"
config="${AKBC_CONFIG:-configs/experiment-qwen-3.5-27b-bf16-thinking-empty-aware-20-think-sample.yaml}"
subjects_file="scripts/h100-bf16/think-sample-subjects.json"
...
python3 - "${subjects_file}" "${dataset_dir}/data/val.jsonl" "${input_path}" <<'PY'
import json, sys
...
with open(val_path, encoding="utf-8") as stream:
    for line in stream:
        row = json.loads(line)
        if row["Relation"] == "hasArea" and row["SubjectEntity"] in subjects:
            selected.append(row)
missing = subjects - {row["SubjectEntity"] for row in selected}
if missing:
    raise SystemExit(f"subjects not found in validation split: {sorted(missing)}")
...
echo "think-sample wall time: ${elapsed} s"
echo "think-sample completed: outputs/${model_key}-candidates.jsonl"

...
config="${AKBC_CONFIG:?set AKBC_CONFIG}"
subjects_file="${AKBC_SUBJECTS:-scripts/h100-bf16/screening-subjects.json}"
...
python3 -m akbc_baseline.download_gguf \
  ...
  --minimum-gib "${AKBC_GGUF_MIN_GIB:-35}" \
  --maximum-gib 158
if [ -n "${gguf_part2}" ]; then
python3 -m akbc_baseline.download_gguf \
  --repo "${gguf_repo}" \
  --filename "${gguf_part2}" \
  --revision "${gguf_revision}" \
...
if [ -n "${AKBC_INPUT_FILE:-}" ]; then
  cp "${AKBC_INPUT_FILE}" "${input_path}"
  echo "using custom input: ${AKBC_INPUT_FILE} ($(wc -l < "${input_path}") rows)"
else
split_path="${dataset_dir}/data/val.jsonl"
case "${subjects_file}" in ALLTEST|ALLTESTAREA|ALLTESTCAP|TESTREL:*) split_path="${dataset_dir}/data/test.jsonl";; esac
python3 - "${subjects_file}" "${split_path}" "${input_path}" <<'PY'
import json, sys

subjects_file, val_path, output_path = sys.argv[1:4]
subjects = None
if subjects_file not in ("ALL", "ALLVAL", "ALLTEST", "ALLCAP", "ALLTESTAREA", "ALLTESTCAP") and not subjects_file.startswith(("REL:", "TESTREL:")):
    with open(subjects_file, encoding="utf-8") as stream:
        subjects = set(json.load(stream)["subjects"])
selected = []
with open(val_path, encoding="utf-8") as stream:
    for line in stream:
        row = json.loads(line)
        if subjects_file in ("ALLVAL", "ALLTEST"):
            selected.append(row)
        elif subjects_file in ("ALLCAP", "ALLTESTCAP"):
            if row["Relation"] == "hasCapacity":
                selected.append(row)
        elif subjects_file == "ALLTESTAREA":
            if row["Relation"] == "hasArea":
                selected.append(row)
        elif subjects_file.startswith(("REL:", "TESTREL:")):
            if row["Relation"] == subjects_file.split(":", 1)[1]:
                selected.append(row)
        elif row["Relation"] == "hasArea" and (
            subjects is None or row["SubjectEntity"] in subjects
        ):
            selected.append(row)
expected = {"ALLVAL": 478, "ALLTEST": 477, "ALLCAP": 100, "ALLTESTAREA": 100, "ALLTESTCAP": 100}.get(subjects_file, 100 if subjects is None else len(subjects))
if subjects_file.startswith(("REL:", "TESTREL:")):
    expected = len(selected)
    if expected == 0:
        raise SystemExit(f"no rows for relation {subjects_file}")
...
for (( gpu = 0; gpu < num_gpus; gpu++ )); do
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" /app/llama-server \
    ...
    --ubatch-size 512 \
    --jinja \
    ${AKBC_REASONING_FLAGS:---reasoning on --reasoning-format none --reasoning-budget 2048 --reasoning-budget-message "The reasoning budget is exhausted. Give the requested JSON array now using the best answer found so far."} \
    --log-verbosity 4 \
    --metrics \
    --perf \
...
echo "model-screening wall time: ${elapsed} s"
echo "model-screening completed: outputs/${model_key}-candidates.jsonl"

#!/usr/bin/env bash
# Multi-route cross-consensus experiment on a multi-GPU host: all 200 numeric
# validation rows (hasArea + hasCapacity) x 20 candidates, four prompted
# reasoning routes x five samples each, raw thinking persisted. One llama-server per GPU (the BF16 27B fits on a
# single 80 GB device), AKBC_CLIENTS_PER_GPU shard clients per server.
set -Eeuo pipefail

cd /opt/akbc

model_key="${AKBC_MODEL_KEY:-gemma-4-31b-multiroute-area}"
config="${AKBC_CONFIG:-configs/experiment-gemma-4-31b-bf16-multiroute-area.yaml}"
dataset_ref="30d8cfaaa7af5b236054cfb361f57b7d0c1e6e57"
gguf_repo="unsloth/gemma-4-31B-it-GGUF"
gguf_revision="c1ac76e99d5513b141e8adde7288b85c3f9c32ec"
gguf_part1="BF16/gemma-4-31B-it-BF16-00001-of-00002.gguf"
gguf_part2="BF16/gemma-4-31B-it-BF16-00002-of-00002.gguf"
...
python3 -m akbc_baseline.download_gguf \
  ...
  --cache-dir "${hf_home}" \
  --path-output "${run_dir}/reports/gemma-gguf-part1-path.txt" \
  --minimum-gib 40 \
  --maximum-gib 52
python3 -m akbc_baseline.download_gguf \
  --repo "${gguf_repo}" \
  ...
  --revision "${gguf_revision}" \
  --cache-dir "${hf_home}" \
  --path-output "${run_dir}/reports/gemma-gguf-part2-path.txt" \
  --minimum-gib 8 \
  --maximum-gib 15
...
echo "gemma-multiroute-area wall time: ${elapsed} s"
echo "gemma-multiroute-area completed: outputs/${model_key}-candidates.jsonl"

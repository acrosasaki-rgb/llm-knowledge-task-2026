#!/usr/bin/env bash
# Raw-completion probe for string relations (company, city) on a base model:
# few-shot completion registers, 20 samples per row.
set -Eeuo pipefail
...
python3 -m akbc_baseline.download_gguf \
  ...
  --path-output "${run_dir}/reports/string-gguf-path.txt" \
...
CUDA_VISIBLE_DEVICES=0 /app/llama-server \
  ...
  > "${run_dir}/reports/string-llama-server.log" 2>&1 &
server_pid=$!
...
def sample(prompt, seed):
    body = json.dumps({
        "prompt": prompt, "n_predict": 32, "temperature": 0.6,
...
def parse(relation, text):
    ...
    if not text or "alive" in text.lower() or text.startswith("("):
        return []
    city = text.split(",")[0].strip().rstrip(".")
    return [city] if city and "<" not in city else []

def run_row(row):
    prompt = REGISTERS[row["Relation"]].format(s=row["SubjectEntity"])
    cands, diags = [], []
    for i in range(20):
        ...
        cands.append(parse(row["Relation"], text))
        diags.append({"raw_text": text})
    return {"SubjectEntity": row["SubjectEntity"],
            "Relation": row["Relation"], "Candidates": cands,

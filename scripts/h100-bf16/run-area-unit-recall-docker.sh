...
#   bash scripts/h100-bf16/run-area-unit-recall-docker.sh
set -Eeuo pipefail
...
image_tag="akbc-qwen27b-bf16-val:area-unit-recall"
...
for variable_name in \
  AKBC_MODEL_KEY AKBC_CONFIG AKBC_PARALLEL AKBC_CTX_PER_SLOT HF_TOKEN \
...
docker run "${run_args[@]}" "${image_tag}" \
  /opt/akbc/scripts/h100-bf16/run-area-unit-recall-container.sh

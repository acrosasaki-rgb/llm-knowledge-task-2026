...
model_key="${AKBC_MODEL_KEY:-qwen3.5-27b-bf16-numeric-multiroute}"
config="${AKBC_CONFIG:-configs/experiment-qwen-3.5-27b-bf16-numeric-multiroute.yaml}"
...
with open(val_path, encoding="utf-8") as stream:
    for line in stream:
        ...
        if row["Relation"] in ("hasArea", "hasCapacity"):
            selected.append(row)
if len(selected) != 200:
    raise SystemExit(f"expected 200 numeric rows, found {len(selected)}")
with open(output_path, "w", encoding="utf-8") as stream:
    for row in selected:
        stream.write(json.dumps(row, ensure_ascii=False) + "\n")
print(f"selected {len(selected)} numeric rows")
PY
...
echo "numeric-multiroute wall time: ${elapsed} s"
echo "numeric-multiroute completed: outputs/${model_key}-candidates.jsonl"
